...
export class MemoryCommandHandler extends CommandHandlerEnhanced {
   ...
   * 处理内存相关命令
   */
  async handleMemoryCommand(command: string, args: string[]): Promise<string> {
   ...
   * 列出用户的所有记忆
   */
  private async handleListMemories(userId: string): Promise<string> {
   ...
   * 显示内存统计
   */
  private async handleMemoryStats(userId: string): Promise<string> {
   ...
   * 搜索记忆
   */
  private async handleMemorySearch(userId: string, query: string): Promise<string> {
    if (!query) {
      return "❌ Please provide search query: /memory-search <query>";
    }

    const results = await this.memoryManager.getRelevantMemories(
      userId,
      query,
      10
    );
   ...
   * 清除所有记忆
   */
  private async handleClearMemories(userId: string): Promise<string> {
   ...
   * 导出记忆
   */
  private async handleExportMemory(userId: string): Promise<string> {
   ...
   * 添加事实
   */
  private async handleAddFact(userId: string, fact: string): Promise<string> {
    if (!fact) {
