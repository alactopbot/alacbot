...
export class ConversationAnalyzer {
   ...
   * 提取用户提供的事实
   */
  private async extractUserFacts(userId: string, userMessage: string): Promise<void> {
    const patterns = [
      /my name is (\w+)/i,
      /i'?m (\w+)/i,
      /call me (\w+)/i,
   ...
   * 将提取的匹配转换为事实
   */
  private extractedToFact(match: RegExpMatchArray): string | null {
    const [fullMatch, ...groups] = match;

    // 简单的规则
    if (groups.length === 1) {
      if (
        fullMatch.toLowerCase().includes("name") ||
  ...
  private async extractKeyInformation(
    ...
    if (
      ...
      await this.memoryManager.addShortTermMemory(
        userId,
        `User asked: ${userMessage.substring(0, 150)}...`,
    ...
    if (importance > 70) {
      await this.memoryManager.addLongTermMemory(
        userId,
        `Key point from conversation: ${userMessage.substring(0, 100)}`,
        importance
      );
    }
  }
  ...
  private calculateImportance(userMessage: string, assistantResponse: string): number {
    ...
    // 问题比陈述更重要
    if (userMessage.includes("?")) importance += 10;
