...
export class ModelManager {
    constructor(workspaceManager) {
        this.knownProviders = new Set(getProviders());
        this.currentModel = "";
        ...
        // 设置默认模型
        this.currentProvider = this.resolveProvider(this.config.modelConfig.defaultProvider, this.getFallbackProvider());
    ...
    async validateModels() {
        ...
        for (const provider of this.config.modelConfig.providers) {
            ...
            for (const model of provider.models) {
                console.log(`    • ${model.name} (${model.id}) - ${model.contextWindow.toLocaleString()} tokens`);
            }
        }
        console.log();
    }
    ...
    getProviderConfig(providerName) {
        return this.config.modelConfig.providers.find((p) => p.name === providerName);
    }
    /**
     * 获取模型配置
     */
    ...
    resolveProvider(providerName, fallback) {
    ...
    isKnownProvider(name) {
        return Boolean(name) && this.knownProviders.has(name);
    }
    /**
     * 获取当前模型
     */
    ...
    listProviderModels(providerName) {
        ...
        provider.models.forEach((model, index) => {
            const isCurrent = this.currentProvider === providerName &&
    ...
    switchModel(providerName, modelId) {
        ...
        this.currentProvider = this.resolveProvider(providerName, this.currentProvider);
     ...
     * 快速切换模型
     */
    quickSwitchModel(input) {
        // 尝试直接匹配模型名称或ID
        for (const provider of this.config.modelConfig.providers) {
            for (const model of provider.models) {
                if (model.id.toLowerCase() === input.toLowerCase() ||
     ...
     * 追踪使用成本
     */
    trackUsage(provider, modelId, inputTokens, outputTokens) {
